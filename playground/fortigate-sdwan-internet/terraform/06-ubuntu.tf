...
resource "azurerm_virtual_machine" "lnx_hub_vm" {
  ...
  storage_os_disk {
    name              = "${var.PREFIX}-HUB-LNX-OSDISK"
    caching           = "ReadWrite"
...
#  value = "${data.azurerm_public_ip.lnx_hub_pip.ip_address}"
#}
#
#output "lnx_hub_private_ip_address" {
...
resource "azurerm_virtual_machine" "lnx_branch1_vm" {
  ...
  storage_os_disk {
    name              = "${var.PREFIX}-BRANCH1-LNX-OSDISK"
    caching           = "ReadWrite"
    create_option     = "FromImage"
    managed_disk_type = "Standard_LRS"
  }
...
resource "azurerm_virtual_machine" "lnx_branch2_vm" {
  ...
  storage_os_disk {
    name              = "${var.PREFIX}-BRANCH2-LNX-OSDISK"
    caching           = "ReadWrite"
    create_option     = "FromImage"
    managed_disk_type = "Standard_LRS"
  }
...
#  value = "${data.azurerm_public_ip.lnx_branch1_pip.ip_address}"
#}
...
#  value = "${data.azurerm_public_ip.lnx_branch2_pip.ip_address}"
#}
