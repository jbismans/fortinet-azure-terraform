...
resource "azurerm_network_interface" "fts_ifc_lan_a" {
  name                      = "${var.PREFIX}-FTS-IFC-LAN-A"
...
resource "azurerm_network_interface" "fts_ifc_lan_b" {
  name                      = "${var.PREFIX}-FTS-IFC-LAN-B"
  location                  = "${azurerm_resource_group.resourcegroup.location}"
  resource_group_name       = "${azurerm_resource_group.resourcegroup.name}"
  enable_ip_forwarding      = true
  enable_accelerated_networking   = true
...
resource "azurerm_virtual_machine" "fts_vm" {
  ...
  location              = "${azurerm_resource_group.resourcegroup.location}"
  resource_group_name   = "${azurerm_resource_group.resourcegroup.name}"
  network_interface_ids = ["${azurerm_network_interface.fts_ifc_lan_a.id}", "${azurerm_network_interface.fts_ifc_lan_b.id}", "${azurerm_network_interface.fts_ifc_mgmt.id}"]
